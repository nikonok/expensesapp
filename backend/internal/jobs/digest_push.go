...
func digestPushRun(db *sql.DB, digestSvc *push.DigestService, qhSvc *push.QuietHoursService) Job {
	return func(ctx context.Context) {
		...
		for rows.Next() {
			var sr row
			if scanErr := rows.Scan(&sr.UserID, &sr.Tz, &sr.ReminderTime, &sr.DailyReminder, &sr.FamilyDigest); scanErr != nil {
		...
		for _, sr := range toNotify {
			...
			} else if sr.DailyReminder != 0 {
				digestSvc.SendDailyReminderForUser(ctx, sr.UserID)
			}
		}
