...
test('TC-034 (P1): account description persists after edit', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Edit' }).evaluate(el => (el as HTMLButtonElement).click());
  ...
  await page.getByRole('button', { name: 'Save Changes' }).evaluate(el => (el as HTMLButtonElement).click());
