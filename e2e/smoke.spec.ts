...
test('active tab reflects current route', async ({ page }) => {
  await setup(page);
  await expect(page.locator('button[aria-current="page"]')).toHaveAttribute('aria-label', 'Accounts');

  await goToTab(page, 'Transactions', /\/transactions/);
  await expect(page.locator('button[aria-current="page"]')).toHaveAttribute('aria-label', 'Transactions');
});
