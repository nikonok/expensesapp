...
function buildBuckets(
  ...
  if (scale === "hour") {
    ...
    while (!isAfter(cursor, end)) {
      ...
      const label =
        format(labelStart, "dd.MM") +
        (format(labelStart, "dd.MM") !== format(labelEnd, "dd.MM")
          ? "–" + format(labelEnd, "dd")
          : "");
...
export default function SpendingBarChart({
  ...
  const buckets = useMemo(
    () => buildBuckets(transactions, start, end, scale),
    [transactions, start, end, scale],
  );
  ...
  const labelInterval =
    totalBuckets > 20 ? Math.ceil(totalBuckets / 10) - 1 : totalBuckets > 10 ? 1 : 0;
  ...
  return (
    <div style={{ padding: "0 var(--space-4)", paddingBottom: "var(--space-2)" }}>
      ...
      <ResponsiveContainer width="100%" height={160}>
        ...
        >
          <CartesianGrid vertical={false} stroke="oklch(22% 0.04 265 / 50%)" strokeDasharray="0" />
          <XAxis
            dataKey="label"
            tick={{
              fontFamily: '"DM Sans", sans-serif',
          ...
          >
            {buckets.map((entry, index) => {
              ...
              return (
                <Cell
                  key={`cell-${index}`}
                  fill={isActive ? "var(--color-primary)" : "oklch(72% 0.22 210 / 70%)"}
                  style={{
