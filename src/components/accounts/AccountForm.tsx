...
export default function AccountForm({ isOpen, onClose, editAccount }: AccountFormProps) {
  ...
  useEffect(() => {
    ...
    if (editAccount) {
      ...
      setMortgageTermYears(
        editAccount.mortgageTermYears != null ? String(editAccount.mortgageTermYears) : "",
      );
      setMortgageInterestRate(
        editAccount.mortgageInterestRate != null
          ? String(editAccount.mortgageInterestRate * 100)
          : "",
      );
      setDebtOriginalAmount(
        editAccount.debtOriginalAmount != null ? String(editAccount.debtOriginalAmount / 100) : "",
      );
      setAlreadyPaid("");
  ...
  const numpadFieldConfig: Partial<
    Record<NonNullable<typeof activeNumpadField>, { label: string; suffix?: string }>
  > = {
    startingBalance: {
  ...
  const validate = () => {
    const effectiveDebtOriginalAmount =
      debtInputMode === "alreadyPaid"
    ...
    const raw = {
      ...
      mortgageLoanAmount:
        type === "DEBT" && debtSubtype === "mortgage" && effectiveDebtOriginalAmount
          ...
          : null,
      mortgageStartDate:
        type === "DEBT" && debtSubtype === "mortgage" ? mortgageStartDate || null : null,
      mortgageTermYears:
        type === "DEBT" && debtSubtype === "mortgage" && mortgageTermYears
          ? parseInt(mortgageTermYears)
          : null,
      mortgageInterestRate:
        type === "DEBT" && debtSubtype === "mortgage" && mortgageInterestRate
          ? parseFloat(mortgageInterestRate) / 100
          : null,
      debtOriginalAmount:
        type === "DEBT" && effectiveDebtOriginalAmount
          ? Math.round(parseFloat(effectiveDebtOriginalAmount) * 100)
          : null,
    };
  ...
  const previewLoanStr =
    debtInputMode === "alreadyPaid"
      ? String(parseFloat(alreadyPaid || "0") + parseFloat(startingBalance || "0"))
      : debtOriginalAmount;
  const previewLoanCents = Math.round(parseFloat(previewLoanStr || "0") * 100);
  const previewRate = parseFloat(mortgageInterestRate || "0") / 100;
  const previewTerm = parseInt(mortgageTermYears || "0", 10);
  const previewPayment =
    previewLoanCents > 0 && previewTerm > 0
      ? calculateMortgagePayment(previewLoanCents, previewRate, previewTerm)
      : 0;
      ...
      >
        ...
        >
          <>
                ...
                {(["REGULAR", "DEBT", "SAVINGS"] as AccountType[]).map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      background:
                        type === t ? "var(--color-primary-dim)" : "var(--color-surface-raised)",
            ...
            {!isEdit && (
              <div style={sectionStyle}>
                <span style={labelStyle}>
                  {type === "DEBT" ? "Current Balance (what you owe today)" : "Starting Balance"}
                </span>
                <button
                  onClick={() => {
            ...
            {type === "DEBT" && (
              <>
                ...
                {!isEdit && (
                  <div style={sectionStyle}>
                    ...
                    <button
                      onClick={() => {
                        const fieldKey =
                          debtInputMode === "original" ? "debtOriginalAmount" : "alreadyPaid";
                        const currentVal =
                          debtInputMode === "original" ? debtOriginalAmount : alreadyPaid;
                    ...
                    >
                      ...
                      {formatNumpadDisplay(
                        String(
                          Math.max(
                            0,
                            parseFloat(debtOriginalAmount || "0") -
                              Math.abs((editAccount?.balance ?? 0) / 100),
                          ),
                        ),
                      )}
                    </span>
                ...
                {debtSubtype === "mortgage" && (
                  <>
                    <div style={sectionStyle}>
                      <label htmlFor="mortgage-start" style={labelStyle}>
                        Start Date
                      </label>
                      <input
                        id="mortgage-start"
                    ...
                    {previewPayment > 0 && (
                      ...
                      >
                        {editAccount != null ? "New monthly payment:" : "Monthly payment:"}{" "}
                        <span
                          style={{
                            fontFamily: '"JetBrains Mono", monospace',
                            fontWeight: 500,
                        ...
                        >
                          {formatAmount(previewPayment, currency)}/mo
                        </span>
                      </div>
                    )}
                  </>
            ...
            {/* Description */}
            <div style={sectionStyle}>
              <label htmlFor="acc-desc" style={labelStyle}>
                Description
              </label>
              <textarea
                id="acc-desc"
