...
export function BudgetStats({ categoryId, accountId, month }: BudgetStatsProps) {
  ...
  const stats = useLiveQuery(async () => {
    ...
    if (categoryId != null) {
      allTxs = await db.transactions.where("categoryId").equals(categoryId).toArray();
    ...
    if (categoryId != null) {
      ...
      const budgets = await db.budgets.where("accountId").equals(accountId).toArray();
      budgets.sort((a, b) => b.month.localeCompare(a.month));
      if (budgets.length > 0) lastBudget = budgets[0].plannedAmount;
    }
  ...
  const rows: { label: string; value: string }[] = [
    {
      label: "Avg monthly",
      value: stats?.avgMonthly != null ? formatAmount(stats.avgMonthly, mainCurrency) : "N/A",
    },
    {
      label: `Last month (${format(prevMonth, "MM.yyyy")})`,
      value: stats?.lastMonth != null ? formatAmount(stats.lastMonth, mainCurrency) : "N/A",
    },
    {
      label: "Last set budget",
      value: stats?.lastBudget != null ? formatAmount(stats.lastBudget, mainCurrency) : "N/A",
    },
  ];
